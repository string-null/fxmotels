Init = function()
	FX.UseCallback("fx_motels:fetchMotelRooms", function(fetchedMotels)
	...
	while true do
        if IsEntityPlayingAnim(PlayerPedId(), 'missheistfbisetup1', 'hassle_intro_loop_f', 3) then
            ...
            if math.floor((timer / 1000) / Config.RaidTimer * 100) >= 100 then
				...
				TriggerServerEvent("fx_motels:syncDoorState", roomId, 0)
                break
            end
        else
...
RoomManagment = function(roomId, roomNumber, motelData)
	...
	if Config.EnableKeySystem then
		table.insert(menuElements, {
			["label"] = "Consigue llaves extra $" ..Config.KeyPrice,
			["action"] = "key"
		})
	end
	...
	if motelData["rentMode"] then
		setTime = false
		FX.UseCallback("fx_motels:fetchRentTime", function(fetchedTime, error)
			if fetchedTime then
				...
				local paymentLabel = missedPayment and "" .. (payments > 1 and payments .. " facturas impagas" or payments .." facturas impagas")  or "Siguiente pago " .. (h > 0 and h .. " momento " .. math.floor(m) .. " minuto" or math.floor(m) .. " minuto")
				table.insert(menuElements, {
					["label"] = paymentLabel,
					["time"] = fetchedTime,
	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), "management_menu", {
		...
		if current["action"] == "payment" then
			if current["missedPayments"] > 0 then
				...
				FX.UseCallback("fx_motels:payRent", function(proccesed)
					while not proccesed do
						Citizen.Wait(0)
					end
					RemoveLoadingPrompt()
					-- ESX.ShowNotification("Oda kiranı ödedin. $" ..motelData["motelPrice"] * current["missedPayments"])
				end, roomId, current["missedPayments"], motelData)
			else
				local h, m = ConvertTime(Config.RentTimer - current["time"]) 

				-- ESX.ShowNotification("Alquiler pagado, " .. (h > 0 and h .. "hours and " .. math.floor(m) .. " minutes" or math.floor(m) .. " minutes") .. " to pay.")
			end
		...
		elseif current["action"] == "changeDesign" then
			...
			while cachedData["previewingDesign"] do
				...
				if IsControlJustReleased(0, 74) then
					FX.UseCallback("fx_motels:updateInteriorFinish", function(changed, errorMessage)
						if changed then
				...
				if dstCheck > 10.0 then
					...
					RefreshInterior(previewInterior)
					-- ESX.ShowNotification("You moved to far away.")
					cachedData["previewingDesign"] = false
				end
				ESX.ShowHelpNotification("Presione ~ INPUT_VEH_HEADLIGHT ~ para confirmar la compra o ~ INPUT_CELLPHONE_CANCEL ~ para cancelar.")
			end
			RefreshInterior(previewInterior)
			RemoveLoadingPrompt()
		elseif current["action"] == "currentDesign" then
			-- ESX.ShowNotification("You already have this design.")
		elseif current["action"] == "key" then
			if cachedData["motelRooms"][roomId]["roomOwner"] == ESX.PlayerData["identifier"] then
				FX.UseCallback("fx_motels:canBuyKey", function(buykey)
					if buykey then
						exports["fx-keys"]:AddKey({
...
StashMenu = function(roomId, storageName)
	...
	if Config.StoreCash then
		if cachedData["motelRooms"][roomId] and cachedData["motelRooms"][roomId]["roomData"]["roomStorages"][storageName]["cash"] > 0 then
			table.insert(menuElements, {
				["label"] = "Cantidad: $" .. cachedData["motelRooms"][roomId]["roomData"]["roomStorages"][storageName]["cash"],
				["action"] = "takeCurrency",
				["currency"] = "cash"
			})
		end
	...
	if Config.StoreBlackMoney then
		if cachedData["motelRooms"][roomId]["roomData"]["roomStorages"][storageName]["black_money"] > 0 then
			table.insert(menuElements, {
				["label"] = "Cojer dinero negro: " .. cachedData["motelRooms"][roomId]["roomData"]["roomStorages"][storageName]["black_money"],
				["action"] = "takeCurrency",
				["currency"] = "black_money"
			})
		end
	...
	FX.UseCallback("fx_motels:fetchStorage", function(fetchedStorage, errorMessage)
...
Wardrobe = function()
	FX.UseCallback('esx_eden_clotheshop:getPlayerDressing', function(dressing)
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				FX.UseCallback('_eden_clotheshop:getPlayerOutfit', function(clothes)
					...
					-- ESX.ShowNotification("Kıyafetin değişti.")
					HasLoadCloth = true
...
OpenMenu = function(menuElements, roomId, storageName)
	...
	ESX.UI.Menu.Open("default", GetCurrentResourceName(), "storage_menu", {
		...
		elseif current["action"] == "take" then
			menuHandle.close()
			local input = OpenInput("How much do you want to put in, you have " .. current["amount"], "input")
			local amount = tonumber(input)

			if amount and amount > 0 then
				FX.UseCallback("fx_motels:updateStorage", function(stored, errorMessage)
					if stored then
						-- ESX.ShowNotification("You took " .. amount .. " ".. current["realLabel"])
					else
						-- ESX.ShowNotification(errorMessage)
					end
				end, roomId, {
				...
				-- ESX.ShowNotification("Please choose a correct amount.")
			end
		elseif current["action"] == "insert" then
			...
			if amount and amount > 0 then
				...
				FX.UseCallback("fx_motels:updateStorage", function(stored, errorMessage)
					if stored then
						-- ESX.ShowNotification("You stored " .. amount .. " ".. current["realLabel"])
					else
						-- ESX.ShowNotification(errorMessage)
					end
				end, roomId, {
		...
		elseif current["action"] == "storeCurrency" then
			menuHandle.close()
			local input = OpenInput("How much " .. (current["currency"] == "cash" and "cash do you want to store?" or "black money do you want to store?"), "input")
			local amount = tonumber(input)
			...
			if amount and amount > 0 then
				...
				FX.UseCallback("fx_motels:updateStorage", function(stored, errorMessage)
					if stored then
						-- ESX.ShowNotification("You stored " .. amount .. " ".. (current["currency"] == "cash" and "dollar cash" or "black money"))
					else
						-- ESX.ShowNotification(errorMessage)
					end
				end, roomId, {
		...
		elseif current["action"] == "takeCurrency" then
			menuHandle.close()
			local input = OpenInput("How much " .. (current["currency"] == "cash" and "cash do you want to take?" or "black money do you want to take?"), "input")
			local amount = tonumber(input)
			...
			if amount and amount > 0 then
				...
				FX.UseCallback("fx_motels:updateStorage", function(stored, errorMessage)
					if stored then
						-- ESX.ShowNotification("You took " .. amount .. " ".. (current["currency"] == "cash" and "dollar cash" or "black money"))
					else
						-- ESX.ShowNotification(errorMessage)
					end
				end, roomId, {
