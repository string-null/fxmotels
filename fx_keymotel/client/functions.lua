...
AddKey = function(keyData)
	FX.UseCallback("dav-keys:addKey", function(keys)
		if keys then
...
RemoveKey = function(removeKeyData)
    FX.UseCallback("dav-keys:removeKey", function(removed)
		if removed then
			...
			if menuData["current"]["uuid"] then
				...
				if player ~= -1 then
					...
					if distance < Config.TransferDistance then
						DrawBusySpinner("Onaylama bekleniyor...")
...
TransferKey = function(key, player)
	FX.UseCallback("dav-keys:transferKey", function(transfered)
		
		DrawBusySpinner("Anahtar veriliyor...")
		...
		if transfered then
            ...
            TriggerServerEvent("dav-keys:giveKey", key, player)
            
			FX.UseCallback("dav-keys:fetchKeys", function(fetchedKeys)
				if fetchedKeys then
			...
			DrawBusySpinner("No se pudo dar la clave.")
